///////////////////////////////////////////////////////////////////////////////////////////////
// `Balanced` Strategy for Sphinx AMM                                                        //
///////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                           //
// Sets a symmetrical bid and ask price around a target price in order to maintain an        //
// inventory of base and quote assets at some target ratio. This strategy is ideal for       //
// counteracting impermanent loss and managing inventory risk.                               //
//                                                                                           //
// Based on the influential Avellaneda & Stoikov paper. See more:                            // 
...
#[starknet::interface]
trait IBalancedStrategy<TContractState> {
    ...
    fn get_optimal_bid_ask(self: @TContractState) -> (u32, u32);
...
mod BalancedStrategy {
    ...
    use super::IBalancedStrategy;
    use balanced::avellaneda;
    ...
    use types252::{UD47x28, UD47x28Trait};

    ////////////////////////////////
    // CONSTANTS
    ///////////////////////////////

    const DEFAULT_VOLATILITY_SQ: u256 = 0;
    ...
    struct Storage {
        ...
        // Model assumptions
        // target ratio of base to total assets in pool
        target_inventory_ratio: u256,
        // risk aversion parameter
        risk_aversion: u256,
        // squared volatility of base asset (constant - assumed)
        volatility_sq: u256,
        ...
        // Pool params
        // whether to buy if optimal bid is above market, or sell if optimal ask is below
        swap_inside_limit: bool, 
    ...
    fn constructor(
        ...
        self.swap_inside_limit.write(swap_inside_limit);
    }
    ...
    impl Strategy of IStrategy<ContractState> {
        ...
        fn update_positions(ref self: ContractState) {
            ...
            let (bid, ask) = self.get_optimal_bid_ask();
    ...
    #[external(v0)]
    impl BalancedStrategy of IBalancedStrategy<ContractState> {
        // Contract owner
        ...
        // Get the optimal bid and ask prices, given current inventory and market conditions.
        fn get_optimal_bid_ask(self: @ContractState) -> (u32, u32) {
            let market_manager = IMarketManagerDispatcher{ contract_address: self.market_manager.read() };
            let market_id = self.market_id.read();
            let market_info = market_manager.market_info(market_id);

            // Calculate base and quote asset exposure.
            let bid_order = market_manager.order(self.bid_order_id.read());
            let ask_order = market_manager.order(self.ask_order_id.read());
            let bid_batch = market_manager.batch(self.bid_batch_id.read());
            let ask_batch = market_manager.batch(self.ask_batch_id.read());

            let bid_base = math::mul_div(bid_batch.base_amount, bid_order.amount_in, batch.amount_in);
            let bid_quote = math::mul_div(bid_batch.quote_amount, bid_order.amount_in, batch.amount_in);
            let ask_base = math::mul_div(ask_batch.base_amount, ask_order.amount_in, batch.amount_in);
            let ask_quote = math::mul_div(ask_batch.quote_amount, ask_order.amount_in, batch.amount_in);

            let base_exposure = self.base_reserves.read() + bid_base + ask_base;
            let quote_exposure = self.quote_reserves.read() + bid_quote + ask_quote;

            // Calculate reservation price and optimal spread.
            let curr_price = market_manager.curr_price(market_id);
            let inventory_delta = avellaneda::inventory_delta(
                curr_price,
                market_info.width,
                base_exposure,
                quote_exposure,
                self.target_inventory_ratio.read(),
            );

            let volatility_sq = self.volatility_sq.read();
            let risk_aversion = self.risk_aversion.read();
            let arrival_intensity = self.arrival_intensity.read();

            let reservation_price = avellaneda::reservation_price(
                curr_price, inventory_delta, volatility_sq, risk_aversion
            );
            let optimal_spread = avellaneda::optimal_spread(volatility_sq, risk_aversion, arrival_intensity);

            // Calculate optimal bid and ask prices.
            let half_spread = optimal_spread / 2;
            let bid_limit = price_math::price_to_limit(reservation_price - half_spread, market_info.width);
            let ask_limit = price_math::price_to_limit(reservation_price + half_spread, market_info.width);
